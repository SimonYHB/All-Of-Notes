...
- AutoreleasePool
  ...
  - AutoreleasePoolPush 
    ...
    - 将POOL_BOUNDARY压入Page并返回其地址

  - autoreleasePoolPop

    - 传入POOL_BOUNDARY地址，会release在Page中的对象直到遇到POOL_BOUNDARY
  ...
  - autoreleasePoolPage类
    ...
    - 占用4096字节，除了存放成员变量，剩下空间用来存autorelease对象地址

    - autoreleasePoolPage之间通过双向链表形式连接在一起，当对象地址不够放时，就会放到下一页

    - runloop和autoreleasepool

      - iOS在主线程的runloop中注册了两个observer
      - 第1个observer监听kCFRunLoopEntry事件，调用objc_autoreleasePoolPush
      - 第2个observer监听了kCFRunLoopBeforeWaiting事件，调用objc_autoreleasePoolPop()、objc_autoreleasePoolPush
      - 因此每次在runloop休眠之前会清理本轮的autorelease对象
